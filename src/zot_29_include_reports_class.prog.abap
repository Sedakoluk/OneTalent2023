...
CLASS lcl_main_controller IMPLEMENTATION.
  ...
  METHOD display_grid.
    ...
    FIELD-SYMBOLS: <lt_data> TYPE STANDARD TABLE, "hangi tabloyu assign edersem onun tipinde olusacak bi table
                   <ls_data> TYPE any.

    IF p_sat = 'X'.
      ASSIGN lt_sat_report TO <lt_data>.
      lv_report_name = 'SAT ALV REPORT'.
    ENDIF.
    IF p_sas = 'X'.
      ASSIGN lt_sas_report TO <lt_data>.
      lv_report_name = 'SAS ALV REPORT'.
    ENDIF.
